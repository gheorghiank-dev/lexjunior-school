...
test.describe("Past Simple – Tense Map", () => {
  ...
  test("ruta de badge se încarcă fără erori", async ({ page }) => {
    const response = await page.goto("/grammar/tenses/past-simple/badge");

    // 1) răspunsul HTTP trebuie să fie OK (200–299)
    expect(response?.ok()).toBeTruthy();

    // 2) suntem pe URL-ul corect
    await expect(page).toHaveURL("/grammar/tenses/past-simple/badge");
  });
});
