...
test("Negative: room is gated until theory is completed", async ({ page }) => {
  ...
  await expect(page.getByTestId("ps-room-negative-1")).toBeVisible();
  ...
  await expect(page).not.toHaveURL(/negative\/room-1/i);

  // Ensure theory is opened at least once (this marks theory as completed)
  if (!/negative\/theory/i.test(page.url())) {
