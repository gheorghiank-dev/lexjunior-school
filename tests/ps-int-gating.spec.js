...
test("Interrogative: room is gated until theory is completed", async ({ page }) => {
  ...
  // Confirm path exists
  await expect(page.getByTestId("ps-path-interrogative")).toBeVisible();
  ...
  // Note: clicking a locked room can redirect to the theory page.
  await expect(page).not.toHaveURL(/interrogative\/room-1/i);

  // Ensure theory is opened at least once (this marks theory as completed)
  if (!/interrogative\/theory/i.test(page.url())) {
