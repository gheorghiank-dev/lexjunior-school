...
test("Uses: room is gated until theory is completed", async ({ page }) => {
  ...
  await expect(page.getByTestId("ps-room-uses-1")).toBeVisible();
  ...
  await expect(page).not.toHaveURL(/uses\/room-1/i);

  // Ensure theory is opened at least once (this marks theory as completed)
  if (!/uses\/theory/i.test(page.url())) {
