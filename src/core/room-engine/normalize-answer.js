...
function normalizeNumbersAndTimes(str) {
  ...
  s = s.replace(/\b0?([0-9]|1[0-9]|2[0-3]):00\b/g, (_, h) =>
    String(parseInt(h, 10)),
  );

  // Times with am/pm like "7 a.m.", "7am" -> "7"
  s = s.replace(/\b0?([0-9]|1[0-9]|2[0-3])\s*a\.?m\.?\b/gi, (_, h) =>
    String(parseInt(h, 10)),
  );
  s = s.replace(/\b0?([0-9]|1[0-9]|2[0-3])\s*p\.?m\.?\b/gi, (_, h) =>
    String(parseInt(h, 10)),
  );
  ...
  s = s.replace(/\b0?([0-9]|1[0-9]|2[0-3])\s+oclock\b/g, (_, h) =>
    String(parseInt(h, 10)),
  );
  ...
  for (let i = 0; i < tokens.length; i += 1) {
    ...
    if (next) {
      ...
      if (
        Object.prototype.hasOwnProperty.call(NUMBER_WORD_MAP, hyphenCombined)
      ) {
...
export function normalizeAnswer(value) {
  ...
  // Accept both contracted and full negative forms:
  // "do not" <-> "don't", "does not" <-> "doesn't"
  // Also accept missing apostrophes: "dont", "doesnt"
  base = base
    .replace(/\bdo\s+not\b/g, "don't")
    .replace(/\bdoes\s+not\b/g, "doesn't")
    .replace(/\bdont\b/g, "don't")
    .replace(/\bdoesnt\b/g, "doesn't")
    // common spacing mistakes around apostrophe: do ' nt -> do'nt
    .replace(/\bdo'nt\b/g, "don't")
    .replace(/\bdoes'nt\b/g, "doesn't");
